...
@testset "generalLineSearch" begin
    f(x) = (x-2)^2-1;
    ∇f(x) = 2(x-2);
    α = 0.5;
    ...
    @test generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α) ≈ 2;
    @test ∇f(generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α, ϵ=1e-10)) ≈ 0 atol=1e-10;
    
    f(x) = x^2-1;
    ∇f(x) = 2x;
    α = 1; # keeps oscillating between two solutions
    @test isnan(generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α));
    α = 0.5;
    @test ∇f(generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α, ϵ=1e-10)) ≈ 0 atol=1e-10;
    ∇f(x) = 2x-1; #wrong gradient: should give a warning!
    # @test_logs Logging.Warn generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α);
    @test_logs (:warn,"∇f doesn't seem to be right") generalLineSearch(f,∇f,2.0,getSearchDirection=getDk, getStepSize=α);
    
    
    f(x) = x[1]^2+(x[2]-1)^2;
    ∇f(x) = [2x[1];2(x[2]-1)];
    @test generalLineSearch(f,∇f,[2.0,2.0],getSearchDirection=getDk, getStepSize=α, ϵ=1e-10) ≈ [0,1] atol=1e-10;
